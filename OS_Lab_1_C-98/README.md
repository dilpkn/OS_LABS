# OS_Lab_1, Актуально для: *Коммит [3]
// ----- Задача: -----
...
*Коммит [3] написан на 98-ом стандарте С++ (Ну почти)

Creator -- Полностью работоспособен
Reporter -- Полностью работоспособен (Забыл потестировать сортировку по номеру)
Main -- Полностью работоспособен

Код ~Нормальный, тестов нет, обработка исключений зачаточная (надо бы блоки try-catch вводить...)
Возможны утечки памяти при "издевательстве" над программой (Судя по предупреждениям компилятора)
